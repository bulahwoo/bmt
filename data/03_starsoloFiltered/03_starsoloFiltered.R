...
library(gt)
library(tximport)
setwd("/media/nguyen/Data1/github/bulahwoo/bmt/data/03_starsoloFiltered/")

`%ni%` <- Negate(`%in%`)

ggp_title <- expression(italic(B.)~italic(mori)~"testes")
...
# load dge
dge_raw <- ReadSTARsolo("/media/nguyen/Data1/mao/scseq/bombyx_mori_testes/starsolo/bmori_testes/Solo.out/Gene/raw/")
dge <- ReadSTARsolo("/media/nguyen/Data1/mao/scseq/bombyx_mori_testes/starsolo/bmori_testes/Solo.out/Gene/filtered/")
...
so_pre_emptydrops <-
  ...
  PercentageFeatureSet(pattern = "^KEF65", col.name = "percent.mt")
...
# STARsolo summary
summary_star <- read.table("/media/nguyen/Data1/mao/scseq/bombyx_mori_testes/starsolo/bmori_testes/Solo.out/Gene/Summary.csv", sep="\t")
write.table(summary_star, "./files/summary_star.tsv", quote=F, sep="\t", row.names=F, col.names=c("Measurement", "Value"))

# UMAP
sobj <-
  CreateSeuratObject(counts=dge, min.cells = 3, min.features = 200) %>%
  PercentageFeatureSet(pattern = "^KEF65", col.name = "percent.mt") %>% subset(., nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5) %>%
